...
void Camera::set(float eyeX, float eyeY, float eyeZ, float lookX, float lookY, float lookZ, float upX, float upY, float upZ)
{
	...
	u.set(up.cross(n).x, up.cross(n).y, up.cross(n).z); // 카메라 업벡터와의 내적으로 u구성
	v.set(n.cross(u).x, n.cross(u).y, n.cross(u).z); // u와 n의 내적으로 시점좌표계의 y축
...
void Camera::setModelViewMatrix(void)
{
	...
	glLoadMatrixf(m);//행렬m을 모델뷰행렬에 넣어준다, 내가 행렬을 만들고 그 행렬을 넣어준 것
}
...
void Camera::slide(double du, double dv, double dn) //이동변환
{
	...
	setModelViewMatrix();	//행렬 다시 계산
}

void Camera::rotAxes(Vector3& a, Vector3& b, float angle) //회전변환
